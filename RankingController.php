...
class RankingController extends Controller
{
    ...
    //no comprobado el funcionamiento, GUIA DE LO QUE QUIERO HACER
    public function addSkillPoints(Request $request)
    {
        $request->validate([
            'id' => 'required|integer',
            'id_student' => 'required|integer',
            'id_rank' => 'required|integer',
        ...
        if ($request->points <= $weeklyPoints) {
            $rankingAnalysis = RankingAnalysis::where('id_rank', $request->id_rank)
                ->where('id_student', $request->id_student)
                ->first();

            if ($rankingAnalysis) {
                $skillName = $request->skill;
                $currentPoints = $rankingAnalysis->$skillName;
                $newPoints = $currentPoints + $request->points;
                $rankingAnalysis->$skillName = $newPoints;
                $rankingAnalysis->save();
            }
        }

        return response()->json(['message' => 'Skill points added successfully.']);
    }
